...
let playPauseButton; // Кнопка Play/Pause
...
function setup() {
    ...
    playPauseButton.style('background-color', 'pink'); // Устанавливаем розовый цвет
    ...
    playPauseButton.mouseOver(() => {
        playPauseButton.style('background-color', 'red');
    });
    playPauseButton.mouseOut(() => {
        playPauseButton.style('background-color', 'pink');
    });
}
...
function draw() {
    if (sound.isPlaying()) {
        ...
        let bassEnergy = fft.getEnergy("bass"); // Получаем уровень энергии басовых 
        ...
        for (let i = 0; i < cols; i++) {
            ...
            if(count > 0){
              avg /= count; // Вычисляем среднюю амплитуду
            }
            let barHeight = map(avg, 0, 255, 0, height); // Отображаем высоту
            fill(barColors[i]); // Цвет столбца
            rect(i * barWidth, height - barHeight, barWidth, barHeight); // Рисуем столбцы
        }

        // Обновление массива амплитуд для рисования волны
        let currentAmplitude = amplitude.getLevel(); // Текущий уровень амплитуды
        amplitudes.shift(); // Убираем первый элемент из массива
        amplitudes.push(currentAmplitude); // Добавляем новый элемент в конец

        // Рисование волны амплитуды
        stroke(255, 200); // Устанавливаем цвет и прозрачность волны
        noFill(); // Отключаем заполнение
        beginShape(); // Начинаем фигуру
        for (let i = 0; i < amplitudes.length; i++) {
            let angle = map(i, 0, amplitudes.length, 0, TWO_PI);
            let offset = map(sin(angle * speedSlider.value()), -1, 1, -200, 200);
            let x = i;
            let y = height / 2 + amplitudes[i] * height / 2 + offset;
            vertex(x, y);
        }
        endShape();
        ...
        noiseLine(bassEnergy);
         ...
         // Рисование линий в направлении angleSlider
         stroke(255, 50);
         for (let i = 0; i < 100; i++) {
             let x = width / 2 + noise(progress) * width / 4 * cos(angleSlider.value() * i); // Координата x с шумом
             let y = height / 2 + noise(progress) * height / 4 * sin(angleSlider.value() * i); // Координата y с шумом
             
              let x2 = width / 2 + noise(progress+100) * width / 4 * cos(angleSlider.value() * i + PI); // Координата x с шумом
             let y2 = height / 2 + noise(progress+100) * height / 4 * sin(angleSlider.value() * i + PI);
             line(x, y, x2, y2);
         }
 
        progress+= noiseStep; // Двигаем шум
